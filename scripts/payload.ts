...
export function buildImagesPayload(options: Options): Record<string, unknown> {
...
export function buildResponsesPayload(options: Options): Record<string, unknown> {
  ...
  const content: Array<Record<string, string>> = [{ type: "input_text", text: options.prompt }];

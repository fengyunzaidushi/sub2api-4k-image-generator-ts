...
function parseChoice(raw, choices, flag) {
    ...
    for (let i = 0; i < args.length; i += 1) {
        ...
        switch (arg) {
            ...
            case "--user-agent":
                ...
                break;
            case "--endpoint":
                options.endpoint = parseChoice(nextValue(args, i, arg), ["images", "responses"], arg);
                i += 1;
                break;
            case "--image-model":
            ...
            case "--n":
                ...
                break;
            case "--output-format":
                options.outputFormat = parseChoice(nextValue(args, i, arg), ["png", "jpeg", "webp"], arg);
                i += 1;
                break;
            case "--background":
                options.background = parseChoice(nextValue(args, i, arg), ["auto", "transparent", "opaque"], arg);
                i += 1;
                break;
            case "--moderation":
                options.moderation = parseChoice(nextValue(args, i, arg), ["auto", "low"], arg);
                i += 1;
                break;
            case "--timeout":
